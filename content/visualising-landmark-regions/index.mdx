...
There is no HTML element which will automatically be assigned the `search` role,
so it's normal to add the role to a form element or input field.

```html
<form role="search" aria-labelledby="search-name">
